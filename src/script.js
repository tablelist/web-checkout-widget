(function(window, document, venueConfigs) {
  ...
  function createIframe(config) {
    ...
    iframeEl.setAttribute('src', '@@DOMAIN/book/' + config.venue + '?client=' + config.venue + '-widget&partner=' + (config.partner || 'venueWidget') + '&theme=' + (config.theme || 'tablelist') + '&widget_id=' + config.id + (config.date ? ('&date=' + config.date) : ''));
    iframeEl.className = 'tablelist-iframe';

    var poweredBy = '<p class="widget-powered-by">Powered by <a href="https://www.tablelist.com">Tablelist</a></p>';
